function [t_comm, t_comp_local, t_comp_RSU, LOC_Veh_x, LOC_Veh_y, LOC_Rsu_x, LOC_Rsu_y, Veh_RSU_Dist] ...
    = rate_from_random_position(N_Veh, N_Rsu, N_Line, Each_Road_width, Bp, Distance, alpha_channel, Veh_max_time, max_Veh, RSU_Cpu_num, Rsu_Cpu, Max_distance)
...
data_comm = 10^6*ones(1,N_Veh);

%Each Vehicle's Needed computation cycle(computational intensity)
Veh_CPU_Need_cycle = 1.2.*(0.9+0.2*rand(1,N_Veh)) * 10^9;
...
for i = 1:N_Veh %vehicle : i, RSU : j
    for j = 1:N_Rsu
        ...
        if delay_fun(max_Veh, RSU_Cpu_num(j), t_comp_RSU(i,j), t_comm(i,j)) > Veh_max_time && RSU_Veh_dist(i,j) > Max_distance
            t_comp_RSU(i,j) = Inf;
        end
    end
end
